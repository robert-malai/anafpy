...
current Git for Windows ships one (curl ≥ 8.16) — via `ANAFPY_CURL`. (anafpy's
Windows extension bundle sidesteps both this and the ARM64 KSP case by
carrying its own x64 Schannel curl; the override remains for everything else.)
