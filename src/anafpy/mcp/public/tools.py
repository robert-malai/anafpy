...
def register(mcp: FastMCP, ctx: AppContext) -> None:
    @mcp.tool(
        ...
        description=cleandoc("""
            Look up CUIs (max 100) in ANAF's public taxpayer/VAT registry — no
            auth needed. One call answers, per CUI as of `date` (ISO, default
            today):
            - VAT registration, VAT-on-collection, split-VAT
            - inactive status
            - RO e-Factura register membership
            - general company data

            CUIs with no data come back in `not_found`. The 'RO' VAT prefix is
            tolerated.
        """),
    )
    ...
    @mcp.tool(
        ...
        annotations=READ_ONLY,
        description=cleandoc("""
            Query the Registrul RO e-Factura (opt-in register) for CUIs (max
            100) — no auth needed.

            Mostly relevant for B2G option dates; for a plain 'is this CUI
            e-Factura-registered' check prefer anaf_lookup_taxpayers (its
            `efactura_registered` answers it alongside everything else).
        """),
    )
    ...
    @mcp.tool(
        ...
        annotations=READ_ONLY,
        description=cleandoc("""
            Query ANAF's farmers' special-regime register (art. 315¹) for CUIs
            (max 500) — no auth needed.

            Membership is the record's `registered` boolean: unknown CUIs still
            come back under `found` with empty fields.
        """),
    )
    ...
    @mcp.tool(
        ...
        annotations=READ_ONLY,
        description=cleandoc("""
            Query ANAF's register of cult entities (tax-credit eligible
            religious entities) for CUIs (max 500) — no auth needed. Membership
            is the record's `registered` boolean.
        """),
    )
    ...
    @mcp.tool(
        ...
        description=cleandoc("""
            ...
            (bilanț) for a given year — no auth needed. The indicator set varies
            by statement type (commercial / banking / insurance).
        """),
    )
