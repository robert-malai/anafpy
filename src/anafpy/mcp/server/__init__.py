"""The anafpy MCP server: ANAF operations (e-Factura, e-Transport, SPV, public
lookups) as Cowork skills.

Built on the phase-1 async clients (``DESIGN.md`` §8). Read-only skills (status,
list, download, lookup, validate) are freely callable. Filing — **both OAuth
services** — is two-step: a ``prepare`` tool renders a preview and returns a
confirmation token bound to the exact document bytes and the CIF, and the matching
``submit`` tool files only when handed that token back with the *same* document and
an explicit ``confirm=True``; tokens are single-use, so one approval can never
repeat a non-idempotent upload. Each service takes two prepare shapes: ready-made
XML gated verbatim (``efactura_prepare`` / ``etransport_prepare`` — for invoices
the strongly recommended path whenever upstream invoicing software produced the
document) and structured composition from the client-layer flat models
...
:mod:`.etransport`, :mod:`.public` and :mod:`.spv`, the reference resources live in
